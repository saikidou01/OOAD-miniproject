...
public class SearchController implements ActionListener, ListSelectionListener {
    ...
    public SearchController(Database db) {
        // Set all the booleans for the searchView for the different users (Renter,
        // Landlord, Manager)
        // to false
        this.rentSearch = false;
        this.managerSearch = false;
    ...
    // actionPerformed function to do something if a components addListener is
    // triggered
    @Override
    public void actionPerformed(ActionEvent e) {
        // TODO Auto-generated method stub
        db.initializeConnection(); // initialize the private memeber Database db connection
        // Enter statement below if the display button for the searchView frame is
        // triggered and landlordSearch boolean equals true
        if (e.getSource().equals(sView.getDisplayButton()) && landLordSearch == true) {
            // Call function getLandLordProperties in the Database class and pass in the
            // member variable landlord ID
            // getLandlordProperties returns an ArrayList of type Properties, that returns
            // all the Properties in the database
        ...
        // Enter statment below if the display button for the searchView frame is
        // triggered and managerSearch boolean equals true
        ...
        // Enter statement below if the reset button for the searchView frame is
        // triggered
        ...
        if (sView.getSearchButton() != null) {
            ...
            if (e.getSource().equals(sView.getSearchButton())) {
                ...
                if (rentSearch) {
                    ...
                    // Take the input entries the user put in and use it to initialize a Property
                    // object using the constructo
                    Property requestedPropertyType = new Property(-1, null, sView.getQuadrantInput(),
                            sView.getTypeInput(), sView.getBedsInput(), sView.getBathsInput(),
                            sView.getFurnishedInput(), null, "Active");
                    // Send the property you made with user specifications to the method
                    // getSearchProperties in the Database
                    // class to search the database for properties with the exact same specification
                    // The method will return an arrayList of type Property with properties that are
                    // a match
                    ...
                    // Make a 2D String array for the JTable in the searchView which will contain
                    // the data

...
public class GUIController implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource().equals(mainpage.getLog())) // Login button is pressed, mainpage GUI turns off and login window
                                                     // turns on
        {
            mainpage.setOff();
            login.enableView();
        ...
        } else if (e.getSource().equals(mainpage.getGuest())) // Guest button is pressed, mainpage GUI turns off and
                                                              // search window turns on
        {
            mainpage.setOff();
            search.enableView();
            search.getView().turnOn();
            search.getView().addBackButtonListener(this);
        } else if (e.getSource().equals(mainpage.getRegister()))// Create New login button is pressed, mainpage GUI
                                                                // turns off and register window turns on
        {
            mainpage.setOff();
            rView.turnOn();
        ...
        } else if (e.getSource().equals(mainpage.getSendEmailButton())) // Send Email button is pressed, mainpage GUI
                                                                        // turns off and Email window turns on
        {
            mainpage.setOff();
        ...
        if (emailV != null) // Email window is opened
        {
            ...
            {
                /**
                 * If Send button is pressed and all the fields are filled, then email would be
                 * sent
                 * and email window turns off and mainPage GUI window turns on
                 */
                if (!emailV.getSub().equals("") && !emailV.getBody().equals("") && !emailV.getFrom().equals("")
                        && !emailV.getPID().equals("")) {
                    emailV.showDialog();
                    emailV.destroyFrame();
                    mainpage.turnOn();
                } else {
            ...
            if (e.getSource().equals(emailV.getBackButton())) // back button is pressed then email window turns off and
                                                              // mainPage GUI window turns on
            {
                emailV.destroyFrame();
                mainpage.turnOn();
            }
        ...
        {
            /**
             * If register button is pressed and all the fields are filled and user name is
             * unique, then registeration would be done
             * and register window turns off and mainPage GUI window turns on
             */

            if (!rView.getUsername().equals("") && !rView.getPassword().equals("") && !rView.getFName().equals("")
                    && !rView.getLName().equals("")) {
                ...
                if (userNameExists == false) // if user name is unique and doesnt exist in database
                {
                    ...
                    db.addUser(rView.getUsername(), rView.getFName(), rView.getLName(), rView.getPassword(),
                            rView.getUserType());

...
public class Database {
    ...
    /**
     * Database Constructor reads user password and establishes connection to
     * database
     */
    public Database() {
    ...
    /**
     * initializeConnection() establishes a connection between the program and the
     * database
     */
     ...
     * @param endD
     */
    public void addProperty(String address, String quadrant, String type, int numBedrooms, int numBathrooms,
            String furnished, double fees, String status, int landID, String startD, String endD) {
        try {
            ...
            query = String.format(query, address, quadrant, type, numBedrooms, numBathrooms, furnished, fees, status,
                    landID, startD, endD);
            Statement stmt = dbConnect.createStatement();
            stmt.executeUpdate(query);
        } catch (SQLException e) {
    ...
    public Property getProperty(int propertyID) {
        ...
        try {
            String query = String.format("SELECT * FROM property WHERE Property_ID = %d AND Status = 'Active'",
                    propertyID);
            PreparedStatement stmt = dbConnect.prepareStatement(query);
            line = stmt.executeQuery();
            line.next();
            ...
            prop = new Property(ID, landlordID, address, quadrant, type, numOfBedrooms, numOfBathrooms, furnished,
                    new Fees(0.00, 0, "No", "NULL", "NULL"), status);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return prop;
    ...
    public void updateFeeStatus(int id) {
        try {
            String query = String
                    .format("UPDATE property SET Status = 'Active', FeesPaid = 'yes' Where Property_ID = %d", id);
            Statement stmt = dbConnect.createStatement();
            stmt.executeUpdate(query);
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public void updatePeriodStatus(int id, double amount, int period) {
        try {
            String query = String.format("UPDATE property SET Fees = %f, FeePeriod = %d Where Property_ID = %d", amount,
                    period, id);
            Statement stmt = dbConnect.createStatement();
            stmt.executeUpdate(query);
        } catch (SQLException e) {
    ...
    public void updatePaidProperty(int id, String startDate, String endDate) {
        try {
            String query = String.format(
                    "UPDATE property SET Status = 'Active', StartDate = '%s', EndDate = '%s' Where Property_ID = %d",
    ...
    public void updatePropertyLandLord(int id, String status, String rentDate, int landID) {
        try {
            if (!status.equalsIgnoreCase("rented")) {
                String query = String.format(
                        "UPDATE property SET Status = '%s', RentDate = 'Null' Where Property_ID = %d AND Landlord_ID = %d",
                        status, id, landID);
                Statement stmt = dbConnect.createStatement();
                stmt.executeUpdate(query);
            } else {
                String query = String.format(
                        "UPDATE property SET Status = '%s', RentDate = '%s' Where Property_ID = %d AND Landlord_ID = %d",
                        status, rentDate, id, landID);
                Statement stmt = dbConnect.createStatement();
                stmt.executeUpdate(query);
            }
        } catch (SQLException e) {
    ...
    public void updateProperty(int id, String status, String rentDate) {
        try {
            if (!status.equalsIgnoreCase("rented")) {
                String query = String.format(
                        "UPDATE property SET Status = '%s', RentDate = 'Null' Where Property_ID = %d", status, id);
                Statement stmt = dbConnect.createStatement();
                stmt.executeUpdate(query);
            } else {
                String query = String.format(
                        "UPDATE property SET Status = '%s', RentDate = '%s' Where Property_ID = %d", status, rentDate,
                        id);
                Statement stmt = dbConnect.createStatement();
                stmt.executeUpdate(query);
            }
    ...
    public ArrayList<Property> getSearchProperties(Property p) {
        ...
        try {
            ...
            while (line.next()) {
                ...
                Property prop = new Property(ID, landlordID, address, quadrant, type, numOfBedrooms, numOfBathrooms,
                        furnished, new Fees(50.00, 0, "No", "N/A", "N/A"), status);
                properties.add(prop);
            }
            stmt.close();
        } catch (SQLException e) {
    ...
    public ArrayList<Property> getLandlordProperties(int landLordID) {
        ...
        try {
            ...
            while (line.next()) {
                ...
                Fees fee = new Fees(line.getDouble("Fees"), line.getInt("FeePeriod"), line.getString("FeesPaid"),
                        startDate, endDate);
                Property prop = new Property(ID, landlordID, address, quadrant, type, numOfBedrooms, numOfBathrooms,
                        furnished, fee, status, startDate, endDate, rentDate);
                properties.add(prop);
            }
            stmt.close();
        } catch (SQLException e) {
    ...
    /**
     * Retrieve a list of all of the properties that were added after the user last
     * logged in
     * 
     * @param lastLogin
    ...
    public ArrayList<Property> getNewProperties(String lastLogin) {
        ...
        try {
            ...
            while (line.next()) {
                ...
                String status = line.getString("Status");
                Property prop = new Property(ID, landlordID, address, quadrant, type, numOfBedrooms, numOfBathrooms,
                        furnished, new Fees(0.00, 0, "No", "N/A", "N/A"), status);
                properties.add(prop);
            }
            stmt.close();
        } catch (SQLException e) {
    ...
    public int countListings(String type, String periodStart, String periodEnd) {
        ...
        try {
            ...
            if (type.equalsIgnoreCase("listed")) {
                query = String.format(
                        "SELECT COUNT(StartDate) AS result_count FROM property WHERE StartDate BETWEEN '%s' AND '%s'",
                        periodStart, periodEnd);
            } else if (type.equalsIgnoreCase("rented")) {
                query = String.format(
                        "SELECT COUNT(RentDate) AS result_count FROM property WHERE RentDate BETWEEN '%s' AND '%s'",
                        periodStart, periodEnd);
            } else if (type.equalsIgnoreCase("active")) {
                query = String.format(
                        "SELECT COUNT(StartDate) AS result_count FROM property WHERE StartDate BETWEEN '%s' AND '%s' AND Status = 'Active'",
                        periodStart, periodEnd);
    ...
    /**
     * Retrieve a list of all of the properties that were rented after a certain
     * date
     * 
     * @param periodStart
     * @param periodEnd
    ...
    public ArrayList<Property> getRentedProperties(String periodStart, String periodEnd) {
        ArrayList<Property> properties = new ArrayList<Property>();
        try {
            String query = String.format("SELECT * FROM property WHERE RentDate BETWEEN '%s' AND '%s'", periodStart,
                    periodEnd);
            PreparedStatement stmt = dbConnect.prepareStatement(query);
            line = stmt.executeQuery();
            while (line.next()) {
